...
Write-Log -Message "===== REBRAND PORTAL NEGERI DIMULAI ====="
...
foreach ($file in $htmlFiles) {
    try {
        ...
        $replacements = @(
            @{ Old = "Portal Negeri"; New = "Portal Negeri" },
            @{ Old = "PortalNegeri"; New = "Portal Negeri" },
            @{ Old = "portalnegeri"; New = "portalnegeri" },
            # Social media links
            @{ Old = "https://twitter.com/portalnegeri"; New = "https://twitter.com/portalnegeri" },
            @{ Old = "https://facebook.com/portalnegeri"; New = "https://facebook.com/portalnegeri" },
            @{ Old = "https://instagram.com/portalnegeri"; New = "https://instagram.com/portalnegeri" },
            @{ Old = "https://youtube.com/@portalnegeri"; New = "https://youtube.com/@portalnegeri" },
            @{ Old = "https://linkedin.com/company/portalnegeri"; New = "https://linkedin.com/company/portalnegeri" },
            # Email
            @{ Old = "portalnegeri@gmail.com"; New = "portalnegeri@gmail.com" },
            # Logo references
            @{ Old = 'alt="PortalNegeri"'; New = 'alt="Portal Negeri"' }
        )
...
foreach ($file in $cssFiles) {
    try {
        ...
        # Color replacements
        $colorReplacements = @(
            @{ Old = "#15803D"; New = "#15803D" },          # Primary: kuning -> hijau
            @{ Old = "#15803D"; New = "#15803D" },
            @{ Old = "rgb(255, 204, 0)"; New = "rgb(6, 95, 70)" },
            @{ Old = "#1F5F7F"; New = "#1F5F7F" },          # Secondary: abu -> biru
            @{ Old = "#052E16"; New = "#052E16" },          # Dark: hitam -> darkgreen
            # Additional color mappings if needed
            @{ Old = "#1e2024"; New = "#052E16" }
        )
...
foreach ($file in $packageJsonFiles) {
    try {
        ...
        $replacements = @(
            @{ Old = """name"": ""portalnegeri"""; New = """name"": ""portalnegeri""" },
            @{ Old = """name"": ""portalnegeri-article-generator"""; New = """name"": ""portalnegeri-article-generator""" }
        )
...
$searchPatterns = @("Portal Negeri", "portalnegeri", "PortalNegeri")
$foundIssues = @()
...
$newColors = @("#15803D", "#052E16", "#1F5F7F")
$colorFound = @{}
...
Write-Log -Message ""
Write-Log -Message "Rebrand Portal Negeri selesai ✅"
